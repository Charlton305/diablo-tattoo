...
export default function ContactForm({ variant = 'darker' }: ContactFormProps) {
  ...
  if (status === 'success') {
    return (
      <div className='sticky top-28 bg-green-600 text-white px-4 py-3'>
        <span className='text-sm'>Enquiry sent! We&apos;ll get back to you as soon as possible.</span>
      </div>
    )
  }
  return (
    <form onSubmit={handleSubmit} className='space-y-6'>
      ...
      {status === 'error' && <p className='text-accent text-sm'>{errorMessage}</p>}
